...
pub(crate) fn handle_ship_collisions(
    ...
    mission_query: Single<
        (&mut MissionState, &mut ShipDamageSensorState),
    ...
    let (mut mission_state, mut damage_sensor) = mission_query.into_inner();
    ...
    for ship in &ships {
        for module_entity in &ship.children {
            ...
            runtime_state.needs_attention = true;
            if matches!(
                ship_kinds.get(&ship.entity),
                Some(CollisionShipKind::Player)
            ) {
                damage_sensor.recent_direction = runtime_module.local_position.normalized_or_zero();
                damage_sensor.recent_distance = runtime_module.local_position.length();
                damage_sensor.recent_intensity = Fx::from_num(damage.damage.max(1));
                damage_sensor.recent_timer = Fx::from_num(0.9);
            }
